...
for i in range(len(dummies_col1)):
    ...
    var_count = var_count.withColumn('cumsum',sum('count').over(Window.partitionBy().orderBy().rowsBetween(-sys.maxsize,0)))
    var_count = var_count.withColumn('perc',col('count')/nrow)
    top_factors = var_count.withColumn('cumperc',col('cumsum')/nrow).filter((col('cumperc')<=cum_p)&(col('perc')>p))
    keep_list = top_factors.select(dummies_col1[i]).rdd.flatMap(lambda x:x).collect()
    dummies_factor_dict[dummies_col1[i]] = keep_list
    factor_set[dummies_col1[i]] = var_count.select(dummies_col1[i]).rdd.flatMap(lambda x:x).collect()
...
for i in range(len(dummies_col)):
    air =  air.withColumn(dummies_col[i],when((col(dummies_col[i]).isin(dummy_info['factor_selected'][dummies_col[i]])),col(dummies_col[i])).otherwise(replacement))
